...
signal_summary <- function(df,pos_x,pos_y,pos_z,rorient,access){
                          
                        #Subset dataframe
                         signal_df = df[c(df$posX==pos_x & df$posY==pos_y & df$posZ==pos_z & df$rec_orient == rorient & df$mac == access),]
...
signal_summary(IPS.clean2,pos_x=2,pos_y=13,pos_z=0,rorient=315,access = "00:14:bf:b1:97:8a")
```
...
mac_orient_signal_df = data.frame(matrix(vector(), 840, 8,
                dimnames=list(c(), c("posX","posY","posZ","rec_orient","mac","unique_comb","avg_signal","sd_signal"))),
                stringsAsFactors=F)

mac_orient_signal_df[,c("posX","posY","posZ","rec_orient","mac")]<- df[,c("posX","posY","posZ","rec_orient","mac")]

mac_orient_signal_df$avg_signal = 0
mac_orient_signal_df$sd_signal = 0

mac_orient_signal_df["unique_comb"] =  paste0(as.character(mac_orient_signal_df$posX),as.character(mac_orient_signal_df$posY),as.character(mac_orient_signal_df$posZ),as.character(mac_orient_signal_df$rec_orient),mac_orient_signal_df$mac)
 
m = list()
for(k in 1: nrow(mac_orient_signal_df)){

  m[[k]]= lapply(IPS.clean2[,c("posX","posY","posZ","rec_orient","mac")],
            FUN = signal_summary,
            pos_x=IPS.clean2$posX[k],
            pos_y=IPS.clean2$posY[k],
            pos_z=IPS.clean2$posZ[k] ,
            rorient=IPS.clean2$rec_orient[k],
            #access = IPS.clean2$mac[k],
            df=IPS.clean2
            )
...
mac_orient_signal_df$avg_signal[k]=  m[[k]]$mac[1]
mac_orient_signal_df$sd_signal[k] =  m[[k]]$mac[2]

}
mac_orient_signal_df = unique(mac_orient_signal_df)
