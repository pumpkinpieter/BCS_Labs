...
dat.76 = CBdata.1_10[[4]] %>% select(-`PiS/TiS`)
...
CBData.clean = CBdata.1_10 %>% bind_rows() %>% select(-`PiS/TiS`) %>% filter(Age != 'NA')

identical(filter(CBData.clean, Year == 1976), dat.76)
...
dat.76 %>% summarise(mean(Age))

CBData.clean %>% summarise(mean(Age))
...
#full_join(CBData.clean, dates, by = "Year")
