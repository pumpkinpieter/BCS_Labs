...
#Now write code to get the index of list elemnet(data frame) for year = 1976
...
#assign the data frame for year 1976 to dat.76
dat.76 = CBdata.1_10[[index]]

#Now write code to remove the specified column
dat.76= dplyr::select(dat.76, -'PiS/TiS')
```
...
```{r summary}
#select Age column from dat.76
age_summary1976 = dat.76%>%dplyr::select(Year,Age) %>%
                  group_by(Year) %>%
#summarize function to generate mean and median summary of column 'Age' 
                  summarise(mean = mean(Age,na.rm = TRUE), median = median(Age,na.rm = TRUE))
...
#Remove rows with missing 'Age' value
dat.76.clean <- dplyr::filter(dat.76,!(is.na(dat.76$Age)))

#check count of missing value in Age column
...
```{r}
a_pts = total %>% filter(n>10000)

IPS.data = IPS.data %>% filter(mac%in%a_pts$mac)
