...
5) Now that you have the combined data set for these 10 years, let's compare some basic results to what you found last week. 
...
1. Apply the same `class` conversions you did last week to get these variables into the correct class type. Use `dplyr` functions to complete this operation (there are several ways to do so).
...
4. Note that the `time` variable is in milliseconds.  Use piping (i.e.` %>% `) to transform it into seconds and then convert its class into a time format using the function `as.POSIXct`.
